...
def mergecafs(featurecode,codeno):
    ...
    if codeno == 2:
        ...
        theCommand = 'ffmpeg64.exe -i callMic-'+ featurecode + '.caf -i callSpeaker-' + featurecode + '.caf  -y -filter_complex amix=inputs=2:duration=longest:dropout_transition=3 -ac 2 -ab 64k -f mp3 ./converted/' + outname
    ...
    if codeno == 1:
        ...
        outname = filename.split('-')[1] + '_' + calcTime(filename) + '.mp3'
        theCommand = 'ffmpeg64.exe -i ' + filename + ' -y -filter_complex amix=inputs=1:duration=longest:dropout_transition=3 -ac 2 -ab 64k -f mp3 ./converted/' + outname
